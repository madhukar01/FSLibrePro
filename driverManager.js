...
export default function driverManager(driverObjects, configs)
{
  var drivers = {};
  var required = [
    'setup',
      ...
      function uploadBlob(data, next) {
        if(isBrowser && (_.isArray(data.pages || data.aapPackets))) {
          ...
          */
          ...
          const jsonDataBinary = JSON.stringify(blob, undefined, 4);

          console.time('upload blob elapsed');
          configs[driver].api.upload.blob(jsonDataBinary, CONTENT_TYPE, function (err, result) {
            if (err) {
