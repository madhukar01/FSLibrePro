...
export default class FreeStyleLibreProtocol {
    ...
    async.doWhilst(
        ...
        this.hidDevice.receive((err, buffer) => {
          ...
          if (buffer !== undefined && buffer.length > 0) {
            ...
            if (hidPacket.responseType === COMMAND.TEXT_RESPONSE) {
              ...
              receiveBuffer = Buffer.concat([receiveBuffer, hidPacket.data]);
              ...
              if (atpPacket === null) {
                ...
                receiveBuffer = Buffer.concat([receiveBuffer, atpPacket.data]);
              } else {
                debug(
              ...
              const validAckSequenceNumbers = [this.sequenceTx, ((this.sequenceTx - 1) + 0x100) % 0x100];
      ...
      () => {
        if (responseType === COMMAND.TEXT_RESPONSE) {
          ...
          return !(TEXT_STATUS_REGEX.exec(receiveBuffer.toString()));
      ...
      (err) => {
        ...
        return cb(null, receiveBuffer.toString());
      },
    );
  }
  ...
  static calcCrc32(buffer) {
    ...
    const paddedBuffer = Buffer.alloc((buffer.length + 3) & 0xfffffffc);
    Buffer.from(buffer).copy(paddedBuffer);

    let remainder = 0xffffffff;
    for (let index = 0; index < paddedBuffer.length / 4; index++) {
      let data = paddedBuffer.readUInt32LE(index * 4);
  ...
  static parseAckFrame(buffer) {
    const packet = struct.unpack(
      buffer, 0, ACK_HEADER_FORMAT, ['sequenceRx', 'sequenceTx', 'unknownData'],
    );
    return packet;
  }
  ...
  getDBRecords(command, successCallback, errorCallback) {
    ...
    this.requestTextResponse(command, (data) => {
      ...
      return successCallback(null, dbRecords);
    }, errorCallback);
  }
  ...
  initCommunication(cb) {
    const initFunctions = [
      (callback) => {
        this.sendCommand(COMMAND.INIT_REQUEST_1, null, '', callback);
      },
      (callback) => {
        this.sendAck(INIT_ACK_MAGIC_DATA, callback);
      },
      (callback) => {
        this.sendCommand(COMMAND.INIT_REQUEST_2, null, '', callback);
      },
      (callback) => {
        this.sendCommand(COMMAND.INIT_REQUEST_3, null, '', callback);
      },
      (callback) => {
        this.sendCommand(COMMAND.INIT_REQUEST_4, null, '', callback);
      },
    ];
